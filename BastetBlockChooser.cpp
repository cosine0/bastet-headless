...
namespace Bastet
{
    ...
    BlockType BastetBlockChooser::GetNext(const Well *well, const Queue &q)
    {
        ...
        int pos = find_if(blockPercentages.begin(), blockPercentages.end(),
                          bind2nd(greater_equal<int>(), random() % 100)) - blockPercentages.begin();
    ...
    Searcher::Searcher(BlockType b, const Well *well, Vertex v, WellVisitor *visitor) : _block(b), _well(well),
                                                                                        _visitor(visitor)
    {
        DFSVisit(v);
    }
