...
namespace Bastet
{
    ...
    enum difficulty_t
    {
        difficulty_normal = 0, difficulty_hard = 1, num_difficulties = 2
    };

    class HighScores : public std::vector<HighScore>
    { //a set would not do the right job (think to ties)
    public:
        bool Qualifies(int score);

        int InsertHighScore(int score,
                            const std::string &scorer); //returns position (1 to HowManyHighScores), -1 if you don't make into the list
    };
